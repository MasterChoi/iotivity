...
std::shared_ptr<OCResource> re[2];
class Light
{
public:
...
//                    " for the first time on server with ID: "<< resource->sid()<<std::endl;
        ...
        if(resource && (resource->uri().at(1) == 'a'))
        {
            ...
            if(resourceURI == "/a/light_1" || resourceURI == "/a/light_2")
            {
				if(resourceURI == "/a/light_1")
					std::cout << "=====This is light_1=====\n";
				if(resourceURI == "/a/light_2")
					std::cout << "=====This is light_2=====\n";
                if (resource->connectivityType() & TRANSPORT_TYPE_TO_USE)
                {
                    curResource = resource;
                    // Get the resource host address
					if(resourceURI == "/a/light_1"){
						std::cout << "+++++PUT "<<resourceURI<<"+++++\n";
						re[0] = resource;
					} else if(resourceURI == "/a/light_2"){
						std::cout << "+++++PUT "<<resourceURI<<"+++++\n";
						re[1] = resource;
					}
...
int main(int argc, char* argv[]) {
    std::cout << "remove one findRource\n";
	std::cout << "add cout in onObserve\n";
    ...
    try
    {
		...
		sleep(10);
		std::cout << "### Main "<<re[0]->uri()<<"\n";
		std::cout << "### Main "<<re[1]->uri()<<"\n";
//		getLightRepresentation(re[0]);
//		sleep(10);
		getLightRepresentation(re[1]);
//		sleep(10);
		putLightRepresentation(re[0]);
//		sleep(10);
		getLightRepresentation(re[0]);
//		sleep(10);
		getLightRepresentation(re[1]);
